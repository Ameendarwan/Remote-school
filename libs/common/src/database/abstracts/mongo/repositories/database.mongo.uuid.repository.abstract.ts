import { DATABASE_DELETED_AT_FIELD_NAME } from '@app/common/database/constants/database.constant';
import {
...
> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
  ...
  async findByIdAndUpdate(
    ...
    const update = this._repository
      .findByIdAndUpdate(_id, { $set: payload }, { lean: true, new: true })
      .where(DATABASE_DELETED_AT_FIELD_NAME)
      .exists(false);
    ...
    return update as EntityDocument;
  }

  //NOTE - might need to add filter query on find
  async findAndUpdate(
    find: Record<string, any>,
    payload: UpdateQuery<Entity>,
    options?: IDatabaseManyOptions<ClientSession>,
    ...
    const update = this._repository
      .findOneAndUpdate(find, payload, { lean: true, new: true })
      .where(DATABASE_DELETED_AT_FIELD_NAME)
      .exists(false);
  ...
  async findOneByIdAndSoftDelete<T = EntityDocument>(_id: string): Promise<T> {
    const findOne = this._repository.findByIdAndDelete<EntityDocument>(_id, {
      lean: true,
    });
  ...
  async findOneByIdAndDelete<T = EntityDocument>(
    filterQuery: FilterQuery<T>,
  ): Promise<T> {
    const findOne = this._repository.findOneAndDelete<EntityDocument>(
      filterQuery,
      {
        lean: true,
      },
  ...
  async findOneAndSoftDelete<T = EntityDocument>(_id: string): Promise<T> {
  ...
  async findOneAndDelete<T = EntityDocument>(
    filterQuery: FilterQuery<T>,
  ): Promise<T> {
